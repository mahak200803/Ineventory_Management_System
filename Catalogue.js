...
const Catalogue = () => {
  ...
  const exportToExcel = async (e) => {
    // Extracting specific attributes from each item
    const extractedData = items.map(item => ({
      ...
      manufacturerContact: item.manufacturerContact,
      qty: item.qty,
      descOne: item.descOne,
      descTwo: item.descTwo,
      descThree: item.descThree,
      soldQty: item.soldQty,
      total: item.soldQty*item.price
    }));
  
    // Exporting the extracted data
    exportFromJSON({ data: extractedData, fileName, exportType });
  };
